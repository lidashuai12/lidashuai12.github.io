...
* 重命名一个列 
* 便于计算 
* 紧跟列名，也可以在列名和别名之间加入关键字AS，别名使用双引号(不要使用单引号)，以便在别名中包含空格或特 殊的字符并区分大小写。 （注：在别名中存在空格时一定要加双引号）
* AS 可以省略 
* 建议别名简短，见名知意 
* 举例：

```mysql
SELECT employee_id emp_id FROM employees;  -- 紧跟列名

SELECT last_name AS name, commission_pct comm -- 列名和别名之间加入关键字AS
FROM employees;

SELECT employee_id "员工id" FROM employees; -- 别名使用双引号
```
...
-- 希望对部门去重，但不对工资去重
-- 错误的
SELECT salary DISTINCT department_id FROM employees;-- 报错，对部门重后的行数与工资数不相等
SELECT DISTINCT department_id,salary FROM employees;-- 仅仅是没有报错，但同时对两者去重
```
...
实际问题的解决方案：引入IFNULL(字段名，替换值)

```mysql
SELECT employee_id, salary "月工资", salary * (1 + IFNULL(commission_pct, 0)) * 12 "年工资" FROM employees;
```

这里你一定要注意，在 MySQL 里面， 空值不等于空字符串。一个空字符串的长度是0，而一个空值的长度是空。而且，在 MySQL 里面，空值是占用空间的。
...
相当于给每一行添加一个字段，字段名为“公司名”，值为“小张科技”
...
![](image-20220531195442995.png)

## 5. 运算符的优先级

数字编号越大，优先级越高，优先级高的运算符先进行计算。

![](image-20220531195522668.png)

## 扩展：使用正则表达式查询

![](image-20220531204253508.png)
...
如果没有指定等级，就相当于Restrict方式。 对于外键约束，最好是采用: `ON UPDATE CASCADE ON DELETE RESTRICT` 的方式,该方式表示对主表进行修改的话，从表对修改内容进行同步，但是如果对主表内容进行删除，则不允许对主表的操作。
...
ALTER TABLE 从表名 DROP INDEX 索引名（也就是外键约束名）;
```
...
不建外键约束，你的操作（创建表、删除表、添加、修改、删除）不受限制，要保证数据的 引用完整 性 ，只能`依靠程序员的自觉` ，或者是 `在Java程序中进行限定` 。例如：在员工表中，可以添加一个员工的 信息，它的部门指定为一个完全不存在的部门。
...
外键约束（FOREIGN KEY）不能跨引擎使用（即主表和从表的引擎必须保持一致）。
...
MySQL将用户对数据的访问`限制`在某些数据的结果集上，而这些数据的结果集可以使用视图来实现。用户不必直接查询或操作数据表。这也可以理解为视图具有`隔离性`。视图相当于在用户和实际的数据表之间加了一层虚拟表。
